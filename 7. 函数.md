...
当Python侧调用`get_data()`方法时，返回值（原生C++类型）必须被转换为可用的Pyhton类型。在这个例子中，默认的返回值策略（`return_value_policy::automatic`）使得pybind11获取到了静态变量`_data`的所有权。

当Python垃圾收集器最终删除`_data`的Python封装时，pybind11将试图删除C++实例（通过operator delete()）。这时，这个程序将以某种隐蔽的错误并涉及静默数据破坏的方式崩溃。
...
另一方面，引用策略在多数其他场合并不是正确的策略，忽略所有权的归属可能导致资源泄漏。作为一个使用pybind11的开发者，熟悉不同的返回值策略及其适用场合是非常重要的。下面的表格将提供所有策略的概览：
...
在技术层面，上述代码会将策略同时应用于getter和setter函数，但是setter函数并不关心返回值策略，这样仅仅出于语法简洁的考虑。或者，你可以通过`cpp_function`构造函数来传递目标参数：
```c++
class_<MyClass>(m, "MyClass")
    .def_property("data"
...
2. 下一节将讨论上面表格之外的调用策略。调用策略表明了涉及返回值和函数参数的引用关系。
3. 可以考虑使用智能指针来代替复杂的调用策略和生命周期管理逻辑。智能指针会告诉你一个对象是否仍被C++或Pyhton引用，这样就可以消除各种可能引发crash或未定义行为的矛盾。对于返回智能指针的函数，没必要指定返回值策略。
...
当一个C++容器对象包含另一个C++对象时，我们需要使用该策略。`keep_alive<Nurse, Patient>`表明至少在索引Nurse被回收前，索引Patient应该被保活。参数索引从1开始，0表示返回值。1表示隐含的this指针，而常规参数索引从2开始。当Nurse的值在运行前被检测到为None时，调用策略将什么都不做。

当nurse不是一个pybind11注册类型时，实现依赖于创建对nurse对象弱引用的能力。当nurse对象不是pybind11注册类型，且不支持弱引用时，将会抛出异常。

如果你使用一个不正确的参数索引，将会抛出"Could not cativate keep_alive!"警告的运行时异常。这时，你应该review你使用的索引。
...
Python提供了一个有用的机制，就是函数可以接收任意数量的参数和关键字参数：
```python
...
这个例子里，必须保证SomeType类型必须已经被binding了（通过py::class_<SomeType>），不然就会抛出异常。

另一个值得注意的事情是，生成的函数签名将使用对象的`__repr__`方法来处理默认参数值。如果对象没有提供该方法，那么函数签名将不能直观的看出默认参数值。
...
要处理这个问题，我们可以定义`SomeType.__repr__`方法，或者使用`arg_v`给默认参数手动添加方便阅读的注释。
```c++
...
注意，这个特性不能与`py::args`一起使用。

### 7.7 Positional-only参数

python3.8引入了Positional-only参数语法，pybind11提供了`py::pos_only()`来提供相同的功能：
```c++
m.def("f", [](int a, int b) { /* ... */ },
...
有时这种转换并不是我们期望的，我们可能更希望绑定代码抛出错误，而不是转换参数。我们可以通过`py::arg`来调用`.noconvert()`方法来实现这个事情。
```c++
...
当函数接受由`py::class_`注册的C++类型的指针或shared holder(如指针指针等)，pybind11允许将Pyhton的None传递给函数，等同于C++中传递nullptr给函数。
